...
{ /* char is stored at left and stretchindex at buffer's right */
	int i = 0;
	char stretch = ' ';
	...
	int length, char stretch, char extra_c)
{
	...
	if (width > length)
	{
		for (i = 1; i < width - length + 1; i++)
			buffer[i] = stretch;
		buffer[i] = '\0';
		if (flags & F_MINUS && stretch == ' ')
			{
			if (extra_c)
				buffer[--index] = extra_c;
			return (write(1, &buffer[index], length) + write(1, &buffer[1], i - 1));
		}
		else if (!(flags & F_MINUS) && stretch == ' ')
			{
			if (extra_c)
				buffer[--index] = extra_c;
			return (write(1, &buffer[1], i - 1) + write(1, &buffer[index], length));
		}
		else if (!(flags & F_MINUS) && stretch == '0')
		{
			if (extra_c)
				buffer[--stretch_start] = extra_c;
			return (write(1, &buffer[stretch_start], i - stretch_start) +
				write(1, &buffer[index], length - (1 - stretch_start)));
		}
	}
	if (extra_c)
		buffer[--index] = extra_c;
	...
	int flags, int width, int precision, int size)
{
	...
	if (width > length)
	{
		...
		if (flags & F_MINUS) /* Asign extra char to left of buffer [buffer>stretch]*/
		{
			return (write(1, &buffer[index], length) + write(1, &buffer[0], i));
		}
		else /* Asign extra char to left of stretching [stretch>buffer]*/
		{
 ...
 * @stretch: Char representing the stretching
 * @extra_c: Char representing extra char
 * @stretch_start: index at which stretching should start
 *
 * Return: Number of written chars.
 */
	...
	int width, int flags, char stretch, char extra_c, int stretch_start)
{
	...
	if (width > length)
	{
			...
			/* extra char to left of buffer */
		{
			...
			buffer[2] = 'x';
			return (write(1, &buffer[stretch_start], i - stretch_start) +
				write(1, &buffer[index], length - (1 - stretch_start) - 2));
		}
	}
	buffer[--index] = 'x';
	buffer[--index] = '0';
